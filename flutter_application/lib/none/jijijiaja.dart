...
//Traer varios datos de la base de datos
/*

Widget futureBuilder() {
  return FutureBuilder(
    future: Future.wait([getUser(), getEmail()]),
    builder: (context, snapshot) {
      if (snapshot.hasData) {
        List<dynamic> userData = snapshot.data![0];
        List userEmails = snapshot.data![1];
        return ListView.builder(
          itemCount: userData.length,
          itemBuilder: (context, index) {
            return Text(
              userData[index]['nombre'] + ' ' + userData[index]['correo'] + ' - ' + userEmails[index],
            );
          },
        );
      } else {
...
QuerySnapshot queryUser = await collectionReferenceUser.where('correo', isNotEqualTo: '').get();
  queryUser.docs.forEach((documento) {
    emails.add(documento['correo']);
  });
  return emails;
}
